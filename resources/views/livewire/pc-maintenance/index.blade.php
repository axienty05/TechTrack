<div class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight flex items-center gap-2">
                <x-mary-icon name="o-computer-desktop" class="w-8 h-8 text-primary" />
                PC Maintenance
            </h1>
            <p class="text-sm opacity-60 mt-1">Jadwal Pemeliharaan Komputer PT. Aneka Coffee Industry</p>
        </div>
        <div class="flex items-center gap-2 flex-wrap">
            ...
            <button wire:click="openCreateDeviceModal" class="btn btn-primary btn-sm gap-2 shadow-lg">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-base-100 rounded-2xl shadow-sm border border-base-content/5 p-4 flex items-center gap-3">
            <div class="bg-primary/10 rounded-xl p-3">
                <x-mary-icon name="o-building-office" class="w-6 h-6 text-primary" />
            </div>
            <div>
                <div class="text-2xl font-bold text-primary">{{ $totalKantor }}</div>
                <div class="text-xs opacity-60">PC Unit Kantor</div>
            </div>
        </div>
        <div class="bg-base-100 rounded-2xl shadow-sm border border-base-content/5 p-4 flex items-center gap-3">
            <div class="bg-secondary/10 rounded-xl p-3">
                <x-mary-icon name="o-building-storefront" class="w-6 h-6 text-secondary" />
            </div>
            <div>
                <div class="text-2xl font-bold text-secondary">{{ $totalPabrik }}</div>
                <div class="text-xs opacity-60">PC Unit Pabrik</div>
            </div>
        </div>
        <div class="bg-base-100 rounded-2xl shadow-sm border border-base-content/5 p-4 flex items-center gap-3">
            ...
            <div>
                <div class="text-2xl font-bold text-success">{{ $completedCurrentPeriod }}</div>
                <div class="text-xs opacity-60">Selesai Periode Ini</div>
            </div>
        </div>
        <div class="bg-base-100 rounded-2xl shadow-sm border border-base-content/5 p-4 flex flex-col justify-center gap-1">
            <div class="flex items-center justify-between">
                <span class="text-xs opacity-60">Progres</span>
            ...
            <div class="text-xs opacity-50">{{ ucfirst($activeTab) }}: {{ $completedCurrentPeriod }}/{{ $totalActiveTab }}</div>
        </div>
    </div>
    ...
    {{-- FILTER BAR --}}
    {{-- ======================================================= --}}
    <div class="bg-base-100 rounded-2xl shadow-sm border border-base-content/5 p-4">
        {{-- Tab Kantor / Pabrik --}}
        <div class="tabs tabs-boxed bg-base-200/50 mb-4 w-fit">
        ...
        <div class="flex flex-wrap gap-3 items-center">
            ...
            <input type="month" wire:model.live="selectedPeriod"
                ...
                title="Pilih Periode" />
            {{-- Export --}}
            <button wire:click="exportExcel" class="btn btn-success btn-sm gap-2">
    ...
    {{-- ======================================================= --}}
    <x-mary-modal wire:model="showMaintenanceModal" class="backdrop-blur-sm" box-class="max-w-2xl p-6">
        ...
        <form wire:submit="saveMaintenance" class="space-y-4">
            ...
            <div>
                ...
                <div class="bg-base-200/50 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            {{-- User Sign --}}
            <div class="bg-base-200/50 rounded-xl p-4 flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                <div class="flex-1">
                ...
                <div class="form-control">
                    <label class="label cursor-pointer gap-2 mt-4 sm:mt-0">
            ...
            <div class="flex items-center justify-end gap-2 pt-4 border-t border-base-content/10">
                <button type="button" wire:click="$set('showMaintenanceModal', false)" class="btn btn-ghost btn-sm">Batal</button>
                <button type="submit" class="btn btn-primary btn-sm px-6 shadow-md">
    ...
    {{-- ======================================================= --}}
    <x-mary-modal wire:model="showHistoryModal" class="backdrop-blur-sm" box-class="max-w-2xl p-6">
        @if($historyDevice)
            <h3 class="font-bold text-lg mb-1 flex items-center gap-2">
                <x-mary-icon name="o-clock" class="text-info w-6 h-6" />
                PC Health History
            </h3>
            ...
            @if($historyDevice->maintenanceRecords->isEmpty())
                <div class="text-center py-8 opacity-40">
                    ...
                    <p>Belum ada riwayat maintenance untuk perangkat ini</p>
                </div>
            @else
                <div class="space-y-3 max-h-96 overflow-y-auto pr-1">
                    @foreach($historyDevice->maintenanceRecords as $rec)
                        ...
                        @endphp
                        <div class="border border-base-content/10 rounded-xl p-4">
                            <div class="flex items-center justify-between mb-2">
                                <div class="flex items-center gap-2">
                                    <x-mary-icon name="{{ $condIcon }}" class="w-5 h-5 text-{{ $condColor }}" />
                                    <span class="font-bold text-sm">{{ $rec->maintenance_date->format('d F Y') }}</span>
                                    <span class="badge badge-{{ $condColor }} badge-sm">
                                ...
                                <div class="flex flex-wrap gap-1 mb-2">
                                    @foreach($checklistLabels as $key => $label)
                                        @if(isset($checklist[$key]))
                                            <span class="badge badge-sm {{ $checklist[$key] ? 'badge-success' : 'badge-ghost opacity-40' }} gap-1">
                            ...
                            @endif
                            @if($rec->notes)
                                <div class="text-xs bg-base-200 rounded-lg p-2 opacity-80">
                                    {{ $rec->notes }}
                                </div>
                            @endif
                        </div>
            ...
            <div class="flex justify-end mt-4">
                <button type="button" wire:click="$set('showHistoryModal', false)" class="btn btn-ghost btn-sm">Tutup</button>
            </div>
        @endif
    </x-mary-modal>
    ...
    {{-- ======================================================= --}}
    <x-mary-modal wire:model="showDeviceModal" class="backdrop-blur-sm" box-class="max-w-lg p-6">
        <h3 class="font-bold text-lg mb-4 flex items-center gap-2">
            <x-mary-icon name="o-computer-desktop" class="text-warning w-6 h-6" />
            {{ $editingDeviceId ? 'Edit Data PC' : 'Tambah PC Baru' }}
        </h3>
        <form wire:submit="saveDevice" class="space-y-4">
            ...
            <div class="flex items-center justify-end gap-2 pt-4 border-t border-base-content/10">
                <button type="button" wire:click="$set('showDeviceModal', false)" class="btn btn-ghost btn-sm">Batal</button>
                <button type="submit" class="btn btn-primary btn-sm px-6 shadow-md">
                    <x-mary-icon name="o-check" class="w-4 h-4" />
                    Simpan
                </button>
    ...
    {{-- MODAL: IMPORT DATA --}}
    {{-- ======================================================= --}}
    <x-mary-modal wire:model="showImportModal" class="backdrop-blur-sm" box-class="max-w-lg p-6">
        <h3 class="font-bold text-lg mb-2 flex items-center gap-2">
            <x-mary-icon name="o-arrow-up-tray" class="text-info w-6 h-6" />
            Import Data PC (Copy Paste dari Excel)
        </h3>
        ...
        <div class="space-y-3">
            ...
            <div>
                ...
                <textarea wire:model="importRawText" rows="10" class="textarea textarea-bordered w-full text-sm font-mono"
                    placeholder="Stephen&#9;DESKTOP-RU1L62Q&#9;IT&#10;Inge&#9;ACCT-01&#9;FA&#10;Lusi&#9;FIN-01&#9;FA"></textarea>
            </div>
            <div class="flex items-center justify-end gap-2 pt-2">
                <button type="button" wire:click="$set('showImportModal', false)" class="btn btn-ghost btn-sm">Batal</button>
                <button wire:click="processImport" class="btn btn-info btn-sm px-6">
                    <x-mary-icon name="o-arrow-up-tray" class="w-4 h-4" />
                    Proses Import
                </button>
            </div>
